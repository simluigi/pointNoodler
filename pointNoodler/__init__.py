...
-Align the noodle with the given pointList.  (need to fix offset calculation)
...
def pointNoodler(pointList):    
    ...
    noodle = createNoodle(numSections)
    objNoodle = om.MObject()
    getMObject('pNoodle1Shape', objNoodle)
    ...
    while not vit.isDone():
        ...
        k = int(round(p.x) + mod)           # rounds p to the nearest decimal digit and converts to int format
    ...
    for section in xrange(numSections): 
        ...
        # plugs in all the values from vectors/points and generates a 4x4 matrix (adjusting o.x with offset) 
        matrix = om.MMatrix()
        ...
        for index in sectionIndices[section]:
            noodlePoints.set(noodlePoints[index] * matrix, index)    
...
def deleteNoodles():
    ...
    noodleList = cmds.ls('pNoodle*')     # list of all cylinders generated by PolyCylinderOnX
    if len(noodleList) > 0:                # delete all if 1 or more cylinders are found (based off list length)
